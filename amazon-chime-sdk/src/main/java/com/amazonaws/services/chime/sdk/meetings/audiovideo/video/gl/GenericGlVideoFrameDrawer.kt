 ...
 *     * Use [SAMPLER_NAME] to access the input texture at the specified coordinates.  This will be translated into an
 *       appropriate function call depending on the input.
 *  The simplest possible implementation can be seen in [DefaultGlVideoFrameDrawer] which simply draws opaque quads
 */
open class GenericGlVideoFrameDrawer(
        private val inputFragmentBase: String
): GlVideoFrameDrawer {
    /**
    ...
    private fun prepareShader(
        ...
        if (currentShaderType != shaderType) {
            ...
            currentShaderType = shaderType.also {
                fragmentShader = createFragmentShaderString(it)
            }
            if (program != -1) {
            ...
            textureCoordinateLocation = GLES20.glGetAttribLocation(program, VERTEX_SHADER_COORDINATE_NAME)
    ...
    private fun createFragmentShaderString(shaderType: ShaderType): String {
        ...
        stringBuilder.append("varying vec2 $INPUT_TEXTURE_COORDINATE_NAME;\n")
        if (shaderType === ShaderType.YUV) {
            stringBuilder.append("uniform sampler2D $INPUT_TEXTURE_Y_NAME;\n")
            stringBuilder.append("uniform sampler2D $INPUT_TEXTURE_U_NAME;\n")
            stringBuilder.append("uniform sampler2D $INPUT_TEXTURE_V_NAME;\n")

            // Add separate function for sampling texture.
            // yuv_to_rgb_mat is inverse of the matrix defined in YuvConverter.
            stringBuilder.append("vec4 ${SAMPLER_NAME}(vec2 p) {\n")
            stringBuilder.append("  float y = texture2D($INPUT_TEXTURE_Y_NAME, p).r * 1.16438;\n")
            stringBuilder.append("  float u = texture2D($INPUT_TEXTURE_U_NAME, p).r;\n")
            stringBuilder.append("  float v = texture2D($INPUT_TEXTURE_V_NAME, p).r;\n")
            ...
            stringBuilder.append(inputFragmentBase)
        } else {
            val samplerName = if (shaderType === ShaderType.OES) "samplerExternalOES" else "sampler2D"
            stringBuilder.append("uniform ").append(samplerName).append(" ${INPUT_TEXTURE_NAME};\n")

            // Update the sampling function in-place.
            stringBuilder.append(inputFragmentBase.replace("${SAMPLER_NAME}(", "texture2D(${INPUT_TEXTURE_NAME}, "))
        }
        return stringBuilder.toString()
    }

    companion object {
        val INPUT_TEXTURE_COORDINATE_NAME = "vTextureCoordinate"
        val SAMPLER_NAME = "sample"
        ...
        // Additional vertex shader input names
        private val INPUT_VERTEX_COORDINATE_NAME = "aPosition"
        private val VERTEX_SHADER_COORDINATE_NAME = "aTextureCoordinate"
        private val TEXTURE_MATRIX_NAME = "uTextureMatrix"
        // Pass through vertex shader.  This class assumes most video
        // processing use cases do not require control over vertex shader
        private val VERTEX_SHADER =
                """
        varying vec2 $INPUT_TEXTURE_COORDINATE_NAME;
        attribute vec4 $INPUT_VERTEX_COORDINATE_NAME;
        attribute vec4 $VERTEX_SHADER_COORDINATE_NAME;
        uniform mat4 $TEXTURE_MATRIX_NAME;
        void main() {
            gl_Position = $INPUT_VERTEX_COORDINATE_NAME;
            $INPUT_TEXTURE_COORDINATE_NAME = ($TEXTURE_MATRIX_NAME * $VERTEX_SHADER_COORDINATE_NAME).xy;
        }
        """

        private val INPUT_TEXTURE_NAME = "sTexture"

        // Translated YUV texture names
